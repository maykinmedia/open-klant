...
class DeleteKlantcontactenNotRelatedToZakenTests(TestCase):
    ...
    def test_output_lists_klantcontact_and_related_interne_taak_as_separate_records(
        ...
        self.assertIn(
            f"Record 1234567890 Klantcontact {expected_datum} {expected_datum}",
            output,
        )
        self.assertIn(
            f"Record 9876543210 Interne taak {expected_datum} {expected_datum}",
            output,
        )
    ...
    def test_output_explicitly_lists_all_related_records(self):
        ...
        _assert_record_listed(betrokkene.uuid, "Betrokkene")
        ...
        _assert_record_listed(partij.nummer, "Partij")
    ...
    def test_output_lists_full_partij_cascade_when_partij_becomes_orphaned(self):
        ...
        PersoonFactory.create(partij=partij)
        ...
        _assert_record_listed(vertegenwoordigden.uuid, "Vertegenwoordigden")
        _assert_record_listed(partij.nummer, "Persoon")
    ...
    def test_output_lists_and_deletes_interne_taak_actor_toewijzing(self):
        ...
        expected_datum = "2020-01-01T00:00:00+00:00"
        pattern = (
            rf"Record\s+{re.escape(str(toewijzing.pk))}\s+"
    ...
    def test_partij_identificator_hierarchy_blocks_deletion_with_clear_error(self):
        klantcontact = KlantcontactFactory.create(
            plaatsgevonden_op=datetime(2020, 1, 1, tzinfo=dt_timezone.utc)
        )
        partij = PartijFactory.create(
        ...
        self.assertIn(str(child_identificator.uuid), str(context.exception))
