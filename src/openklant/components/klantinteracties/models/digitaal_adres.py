...
class DigitaalAdres(APIMixin, models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["soort_digitaal_adres"],
                condition=models.Q(is_standaard_adres=True),
                name="unique_default_per_soort",
            )
        ]
    ...
    def save(self, *args, **kwargs):
        if self.is_standaard_adres:
            # Because there can only be one default address, mark all other addresses as
            # non-default
            DigitaalAdres.objects.filter(
                soort_digitaal_adres=self.soort_digitaal_adres, is_standaard_adres=True
