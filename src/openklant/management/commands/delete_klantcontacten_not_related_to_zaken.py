...
RECORD_TYPE_LABELS: dict[type[Model], str] = {
    Klantcontact: "Klantcontact",
    Betrokkene: "Betrokkene",
    ActorKlantcontact: "Actor klantcontact",
    Onderwerpobject: "Onderwerpobject",
    Bijlage: "Bijlage",
    InterneTaak: "Interne taak",
    InterneTakenActorenThoughModel: "Interne taak - toegewezen actor",
    DigitaalAdres: "Digitaal adres",
    Partij: "Partij",
    Rekeningnummer: "Rekeningnummer",
    PartijIdentificator: "Partij identificator",
    CategorieRelatie: "Categorie relatie",
...
class Command(BaseCommand):
    ...
    def build_deletion(
        ...
        try:
            ...
            blocking = sorted(
                f"{RECORD_TYPE_LABELS.get(type(obj), str(type(obj)._meta.verbose_name).capitalize())} "
                f"{self._identificatie(obj, {})}"
    ...
    def _build_records(
        ...
        partij_identificaties = {
            partij.pk: partij.nummer or str(partij.uuid)
            for partij in collector.data.get(Partij, [])
        }

        records = []
        for model, objs in collector.data.items():
            type_label = RECORD_TYPE_LABELS.get(
                model, str(model._meta.verbose_name).capitalize()
            )
            for obj in objs:
    ...
    @staticmethod
    def _identificatie(obj: Model, partij_identificaties: dict[int, str]) -> str:
        ...
        partij_id = getattr(obj, "partij_id", None)
        if partij_id in partij_identificaties:
            return partij_identificaties[partij_id]
        return str(obj.pk)
